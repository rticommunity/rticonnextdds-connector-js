...
be propagated through the ``'error'`` event. If the ``'error'`` event is emitted and the
object that emits it has no attached listeners for the ``'error'`` event, the program will
be terminated with a non-zero error code and the stack trace will be printed. For more
information please refer to the `documentation for the EventEmitter class <https://nodejs.org/api/events.html#events_error_events>`__.
