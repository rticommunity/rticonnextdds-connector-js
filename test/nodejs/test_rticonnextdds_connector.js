...
describe('Connector Tests', function () {
  ...
  it('is possible to obtain the current version of Connector', function () {
    ...
    expect(
      /RTI Connector for JavaScript, version \d+(\.\d+){2}/.test(version)
    ).deep.equals(true)
    // Expect "NDDSCORE_BUILD_<VERSION>_<DATE>T<TIMESTAMP>Z"
    expect(
      /.*NDDSCORE_BUILD_\d+(\.\d+){2,3}_[0-9]{8}T[0-9]{6}Z/.test(version)
    ).deep.equals(true)
    // Expect "NDDSC_BUILD_<VERSION>_<DATE>T<TIMESTAMP>Z"
    expect(
      /.*NDDSC_BUILD_\d+(\.\d+){2,3}[0-9]_[0-9]{8}T[0-9]{6}Z/.test(version)
    ).deep.equals(true)
    // Expect "RTICONNECTOR_BUILD_<VERSION>_<DATE>T<TIMESTAMP>Z"
    expect(
      /.*RTICONNECTOR_BUILD_\d+(\.\d+){2,3}[0-9]_[0-9]{8}T[0-9]{6}Z/.test(version)
    ).deep.equals(true)
  })
